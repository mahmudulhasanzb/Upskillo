...
export default async function CourseDetails(courseDetails) {
  const featuredItems = [
    { icon: Clock, label: courseDetails?.duration || '12h 30m' },
    { icon: BarChart, label: courseDetails?.level || 'Beginner' },
    { icon: BookOpen, label: `${courseDetails?.totalLessons || 24} Lessons` },
    { icon: Users, label: `${courseDetails?.enrollmentCount || 0} Students` },
  ];
  return (
